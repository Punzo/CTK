...
void ctkDICOMQueryTask::run()
{
  ...
  switch(d->QueryLevel)
    ...
    case DICOMLevel::Series:
      if (!d->Query->querySeries(this->taskUID(), d->StudyInstanceUID))
        {
        this->setIsFinished(true);
        emit canceled();
        return;
    ...
    case DICOMLevel::Instances:
      if (!d->Query->queryInstances(this->taskUID(), d->StudyInstanceUID, d->SeriesInstanceUID))
        {
        this->setIsFinished(true);
        emit canceled();
        return;
...
//----------------------------------------------------------------------------
QList<QSharedPointer<ctkDICOMTaskResults>> ctkDICOMQueryTask::taskResults() const
{
  Q_D(const ctkDICOMQueryTask);
  return d->Query->taskResults();
}
...
void ctkDICOMQueryTask::setServer(ctkDICOMServer& server)
{
  ...
  d->Query->setPreferCGET(d->Server->preferCGET());
}
