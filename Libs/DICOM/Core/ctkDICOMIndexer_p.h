...
public:
  ...
  void clear()
  {
    ...
    qDeleteAll(this->TaskResultsList);
  ...
  {
    QMutexLocker locker(&this->Mutex);
    return this->TaskResultsList.size();
  }

  ctkDICOMTaskResults* popTaskResult()
  {
    QMutexLocker locker(&this->Mutex);
    if (this->TaskResultsList.empty())
      {
      return nullptr;
      }

    ctkDICOMTaskResults* taskResults = this->TaskResultsList[0];
    this->TaskResultsList.erase(this->TaskResultsList.begin());
    return taskResults;
  }
  ...
  {
    QMutexLocker locker(&this->Mutex);
    this->TaskResultsList.push_back(taskResult);
    return this->TaskResultsList.size();
  }
  ...
  {
    QMutexLocker locker(&this->Mutex);
    return (this->IndexingRequests.isEmpty() && this->TaskResultsList.isEmpty());
  }
...
public:
  ...
  void pushTaskResults(QList<QSharedPointer<ctkDICOMTaskResults>> taskResultsList);
