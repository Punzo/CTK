...
public:
  ...
  /// Set the item size for grid view mode
  /// \param size The size of each item (width and height)
  void setItemSize(const QSize& size);
  QSize itemSize() const;

  /// Set whether to show detailed text information
  /// \param show If true, shows series description, modality, and instance count
  void setShowDetailedText(bool show);
  bool showDetailedText() const;

  /// Set the thumbnail size within each item
  /// \param size The size of the thumbnail area
  ...
  /// Enable/disable selection checkbox
  /// \param show If true, shows a checkbox for selection
  void setShowSelectionCheckbox(bool show);
  bool showSelectionCheckbox() const;
...
protected:
  ...
  void paintSelection(QPainter* painter, const QRect& rect, const QModelIndex& index, const QStyleOptionViewItem& option) const;
  
  /// Paint the progress indicator
  void paintProgress(QPainter* painter, const QRect& rect, const QModelIndex& index) const;
  
  /// Paint modality badge
  void paintModalityBadge(QPainter* painter, const QRect& rect, const QModelIndex& index) const;

  /// Get the thumbnail rect within the item rect
  QRect thumbnailRect(const QRect& itemRect) const;
  
  /// Get the text rect within the item rect
  QRect textRect(const QRect& itemRect) const;
  
  /// Get the selection checkbox rect
  QRect selectionRect(const QRect& itemRect) const;

  /// Get fallback icon for when thumbnail is not available
  QPixmap getFallbackIcon(const QString& modality) const;

  /// Get color for modality
  QColor getModalityColor(const QString& modality) const;
