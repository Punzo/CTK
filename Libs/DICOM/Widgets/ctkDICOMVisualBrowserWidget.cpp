...
void ctkDICOMVisualBrowserWidgetPrivate::retrieveSeries()
{
  ...
  this->TaskPool->stopAllTasksNotStarted();
  // Stop running tasks, but do not stop the tasks associated to the selected ones
...
void ctkDICOMVisualBrowserWidget::setTaskPool(ctkDICOMTaskPool& taskPool)
{
  ...
  d->ServerNodeWidget->setTaskPool(d->TaskPool);
  d->connectTaskPool();
}
...
{
  Q_D(const ctkDICOMVisualBrowserWidget);
  d->ServerNodeWidget->storageAETitle();
}
...
{
  Q_D(const ctkDICOMVisualBrowserWidget);
  d->ServerNodeWidget->storagePort();
}
...
void ctkDICOMVisualBrowserWidget::onQueryPatient()
{
  ...
  if (filtersEmpty || (d->TaskPool && d->TaskPool->getNumberOfQueryRetrieveServers() == 0))
    {
    d->updateFiltersWarnings();
    }
  else if (d->TaskPool && d->TaskPool->getNumberOfQueryRetrieveServers() > 0)
    {
    ...
    d->TaskPool->queryPatients(QThread::NormalPriority);
...
void ctkDICOMVisualBrowserWidget::updateGUIFromTaskPool(ctkDICOMTaskResults *taskResults)
{
  ...
  d->ProgressFrame->hide();
  d->QueryPatientPushButton->setIcon(QIcon(":/Icons/query.svg"));
  if (!taskResults || taskResults->typeOfTask() != ctkDICOMTaskResults::TaskType::QueryPatients)
    {
    return;
    }
...
void ctkDICOMVisualBrowserWidget::showSeriesContextMenu(const QPoint &point)
{
  ...
  if (selectedSeriesItemWidget == nullptr)
    {
    return;
    }
